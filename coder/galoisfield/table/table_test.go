		...
		for i := 0; i < 32; i++ {
			...
			for j := 0; j < 16; j++ {
				...
				for k := 0; k < 16; k++ {
					...
					list := []item{
						{"a+(b+c)", field.Add(a, field.Add(b, c))},
						{"a+(c+b)", field.Add(a, field.Add(c, b))},
						{"b+(a+c)", field.Add(b, field.Add(a, c))},
						{"b+(c+a)", field.Add(b, field.Add(c, a))},
						{"c+(a+b)", field.Add(c, field.Add(a, b))},
						{"c+(b+a)", field.Add(c, field.Add(b, a))},
					}
					for x := range list {
		...
		for i := 0; i < 32; i++ {
			...
			for j := 0; j < 16; j++ {
				...
				for k := 0; k < 16; k++ {
					...
					list := []item{
						{"a*(b*c)", field.Mul(a, field.Mul(b, c))},
						{"a*(c*b)", field.Mul(a, field.Mul(c, b))},
						{"b*(a*c)", field.Mul(b, field.Mul(a, c))},
						{"b*(c*a)", field.Mul(b, field.Mul(c, a))},
						{"c*(a*b)", field.Mul(c, field.Mul(a, b))},
						{"c*(b*a)", field.Mul(c, field.Mul(b, a))},
					}
					for x := range list {
						for y := x + 1; y < len(list); y++ {
							xitem, yitem := list[x], list[y]
...
func TestGF_Compare(t *testing.T) {
	...
	for _, row := range []testrow{
		{Default, Default, 0},
		{Poly84310_g3, Poly84320_g2, -1},
		{Poly84320_g2, Poly84310_g3, 1},
		{Poly84310_g3, Poly610_g7, 1},
		{Poly610_g7, Poly84310_g3, -1},
		{Poly610_g2, Poly610_g7, -1},
		{Poly610_g7, Poly610_g2, 1},
	} {
